...
class newAppModel extends Component {
  ...
  createCoupon = async () => {
    ...
    let res = await instance.post("change_book/", payload);

    message.success("Status changed successfully");
    if (this.state.radio === "Forward") {
      alert("here");
    } else {
    ...
    return (
      ...
      >
        <div className="px-2 mb-3 mt-3">
          ...
          {this.state.radio === "Forward" && (
            <div className="mt-3">
              <p>Doctor</p>
              <Select
                style={{ width: 120 }}
                onChange={(e) => this.setState({ fdr: e })}
              >
                {this.props.doctors.map((dr) => {
                  return <Option value={dr.id}>{dr.name}</Option>;
                })}
              </Select>
            </div>
          )}
...
const mapDispatchToProps = (dispatch) => {
  return {
    createApp: (index, app) =>
      dispatch({ type: "UPDATE_BOOKING_STATUS", payload: { index, app } }),
  };
};
